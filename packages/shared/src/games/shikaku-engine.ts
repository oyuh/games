...
export interface ShikakuRankedReplayData {
  puzzleTimes: number[];
  solutions: Rect[][];
}

export type ShikakuRankedValidationCode =
  | "invalid-seed"
  | "invalid-difficulty"
  | "invalid-score"
  | "invalid-time"
...
export type ShikakuRankedValidationResult =
  | { ok: true; replayData: ShikakuRankedReplayData }
  | { ok: false; code: ShikakuRankedValidationCode; reason: string };

/**
 * Rebuild a ranked run from the public seed and validate the replay submitted
 * by the client. This is the server-side anti-tamper check: ranked scores must
 * prove they solved the canonical generated puzzles, not a locally edited board.
 */
export function validateRankedShikakuRun(args: {
  seed: number;
...
}): ShikakuRankedValidationResult {
  const { seed, difficulty, score, timeMs, puzzleCount, replayData, timeToleranceMs = 5_000 } = args;

  if (!Number.isInteger(seed) || seed <= 0 || seed > 2_147_483_647) {
    return { ok: false, code: "invalid-seed", reason: "Ranked Shikaku runs require a positive 32-bit seed." };
  }
  if (!isDifficulty(difficulty)) {
    return { ok: false, code: "invalid-difficulty", reason: "Ranked Shikaku runs require a known difficulty." };
  }
  if (!Number.isInteger(timeMs) || timeMs <= 0) {
    return { ok: false, code: "invalid-time", reason: "Ranked Shikaku runs require a positive integer solve time." };
  }
  if (!Number.isInteger(score) || score !== calculateScore(timeMs, difficulty)) {
    return { ok: false, code: "invalid-score", reason: "The submitted score does not match the engine score for this time and difficulty." };
  }
  if (puzzleCount !== PUZZLES_PER_RUN) {
    return { ok: false, code: "invalid-puzzle-count", reason: "Ranked Shikaku runs must complete the full five-puzzle set." };
  }

  const replay = normalizeShikakuReplayData(replayData);
  if (!replay) {
    return { ok: false, code: "invalid-replay", reason: "Ranked Shikaku submissions must include replay rectangles and puzzle split times." };
  }
  if (replay.solutions.length !== PUZZLES_PER_RUN || replay.puzzleTimes.length !== PUZZLES_PER_RUN) {
    return { ok: false, code: "invalid-replay", reason: "The replay does not contain exactly five solved puzzles." };
  }

  const splitTotal = replay.puzzleTimes.reduce((total, value) => total + value, 0);
  if (replay.puzzleTimes.some((value) => !Number.isInteger(value) || value <= 0) || Math.abs(splitTotal - timeMs) > timeToleranceMs) {
    return { ok: false, code: "invalid-time", reason: "The replay split times do not match the submitted total time." };
  }

  const run = generateRun(seed, difficulty);
  if (run.length !== PUZZLES_PER_RUN || run.some((puzzle) => !isRankableShikakuPuzzle(puzzle))) {
    return { ok: false, code: "invalid-generated-run", reason: "The generated run is not eligible for ranked scoring." };
  }

  for (let index = 0; index < run.length; index++) {
    const puzzle = run[index]!;
    const solution = replay.solutions[index]!;
    if (!validateSolution(puzzle, solution)) {
      return { ok: false, code: "non-canonical-solution", reason: `Puzzle ${index + 1} was not solved against the canonical generated board.` };
    }
  }
...
function normalizeShikakuReplayData(value: unknown): ShikakuRankedReplayData | null {
  if (!isRecord(value)) return null;

  const rawTimes = value.puzzleTimes;
  const rawSolutions = value.solutions;
  if (!Array.isArray(rawTimes) || !Array.isArray(rawSolutions)) return null;

  const puzzleTimes = rawTimes.map((time) => Number(time));
  const solutions: Rect[][] = [];
  for (const rawSolution of rawSolutions) {
    const solution = normalizeRectList(rawSolution);
    if (!solution) return null;
    solutions.push(solution);
  }

  return { puzzleTimes, solutions };
}

function normalizeRectList(value: unknown): Rect[] | null {
  ...
  for (const item of value) {
    ...
    if (![r, c, w, h].every(Number.isInteger) || r < 0 || c < 0 || w <= 0 || h <= 0) {
      return null;
    }
    rects.push({ r, c, w, h });
  }
...
function isRankableShikakuPuzzle(puzzle: ShikakuPuzzle): boolean {
  if (!validateSolution(puzzle, puzzle.solution)) return false;
  // The fallback generator emits an all-1x1 board; it is valid but not ranked.
  return getAutoFilledRects(puzzle).length < puzzle.solution.length;
}
